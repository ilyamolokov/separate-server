...
async function parseCurrencyRate() {
    ...
    return parseFirstAPI("USD", "CNY")
        ...
        .catch(async () => JSON.parse(await readFile("server/store/currency/currency.json")).currency)
        .catch(() => console.error("JSON сломан"))
        .then((res) => writeFile("server/store/currency/currency.json", JSON.stringify({ currency: res })))
        .catch(console.error);
}
